...
export default function PlannerForm({ onGenerate, loading }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="pt-2">
        <button type="submit" disabled={loading} className="btn-primary w-full sm:w-auto text-sm py-4 px-8 disabled:opacity-50 disabled:cursor-not-allowed">
          {loading ? 'Building your week...' : 'Generate plan'}
        </button>
      </div>
    </form>
  )
